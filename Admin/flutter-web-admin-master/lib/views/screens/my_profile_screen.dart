...
class _MyProfileScreenState extends State<MyProfileScreen> {
  ...
  _uploadProfilePicToStorage(dynamic image) async {
    Reference ref = _storage.ref().child('AdminProfile').child(fileName!);
 ...
 if(_image != null){
  String imageUrl = await _uploadProfilePicToStorage(_image);

  await _firestore.collection('AdminProfile').doc(fileName).set({
    'image': imageUrl,
  });
 }
              ...
              children: [
                CircleAvatar(
                  ...
                  backgroundImage: _image != null
                      ? MemoryImage(_image)
                      : NetworkImage(_formData.userProfileImageUrl) as ImageProvider,
                  radius: 60.0,
                ),
                Positioned(
                  top: 0.0,
